...
Este documento define el JSON que describe **un examen especial** (Ley 10/2010 y RD 304/2014) **o una evaluación de las operaciones o actividades de un cliente** (AMLR, art. 69.2) en la que puede haber participado un sistema de IA. Un registro puede ser también **una alerta revisada y descartada sin abrir examen especial** (§10): en España no genera expediente, y en el AMLR puede ser, según la lectura, una evaluación del art. 69.2 que hay que registrar ([R-6](ambiguedades.md#r-6)). Solo describe la entrada. Qué se comprueba con ella está en [`especificacion-calculo.md`](especificacion-calculo.md).
...
- Los casos que la norma no resuelve (R-1 a R-11) están en [`ambiguedades.md`](ambiguedades.md). El modelo no los decide: recoge los hechos que hacen falta para aplicar cada lectura.
...
Una alerta de octubre de 2027 que se revisa y se descarta sin abrir examen especial (§10). Con la Ley y el RD no hay expediente; con el AMLR, la lectura AD-2 de [R-6](ambiguedades.md#r-6) la trata como una evaluación que hay que registrar.
...
**Por qué una lista y no un número.** El RD no dice qué año cuenta ni cómo se cuentan las operaciones ([R-7](ambiguedades.md#r-7)). Con una lista, la entrada recoge los años que la entidad conoce y cada lectura elige el suyo. **[Decisión propia]**
...
| `persona_autorizada_por_el_representante` | RD, art. 35.1: «El representante podrá designar, asimismo, hasta dos personas autorizadas que actuarán bajo la dirección y responsabilidad del representante». Ver [R-10](ambiguedades.md#r-10). |
...
| `fecha_cierre` | fecha | sí | RD, art. 25.3: «sus fechas de apertura y cierre». Qué hecho cierra el expediente es [R-9](ambiguedades.md#r-9). |
...
| `participaciones_ia` | lista de `id` de `participaciones_ia` | sí | Si la circunstancia es, o incluye, la salida de un sistema. Puede estar vacía. Si la salida de un sistema debe figurar aquí aunque la entidad diga que no la tuvo en cuenta es [R-1](ambiguedades.md#r-1). |
...
| Valor de `tipo` | Cita |
|---|---|
| `persona` | RD, art. 25.2, párrafo primero: «el representante ante el Servicio Ejecutivo de la Comisión adoptará [...] la decisión». Quién decide con el AMLR es [R-4](ambiguedades.md#r-4); el modelo recoge la persona y sus cargos (§2.2), sin presuponer cuál debe ser. |
...
| `otro_organo_colegiado` | Un órgano distinto del de control interno. Ningún texto lo prevé para esta decisión, pero es un hecho posible, por ejemplo con el AMLR, que no dice quién decide ([R-4](ambiguedades.md#r-4)). **[Decisión propia]** |
...
| `sentido` | `"comunicar"` \| `"no_comunicar"` \| `"abstencion"` | sí | RD, art. 25.2: «el sentido [...] del voto». **[Decisión propia]**: `abstencion` no está en el texto, pero es un hecho que puede constar en un acta. Cómo cuenta para la mayoría es [R-8](ambiguedades.md#r-8). |
...
| `propuesta_decision` | El sistema propone comunicar o no comunicar. AMLR, art. 76.5: «decisiones resultantes de procesos automatizados [...] o de procesos que impliquen sistemas de inteligencia artificial». Si el art. 76.5 alcanza la decisión de comunicar es [R-2](ambiguedades.md#r-2). |
...
- **AMLR.** Art. 77.1.b: «un registro de la evaluación realizada de conformidad con el artículo 69, apartado 2, incluida la información y las circunstancias consideradas y los resultados de dicha evaluación, con independencia de que dicha evaluación dé lugar o no a una comunicación». Si revisar una alerta ya es «evaluar» en el sentido del art. 69.2 es [R-6](ambiguedades.md#r-6), con dos lecturas:
...
| Registro de una alerta revisada y descartada | — (sin examen especial no hay expediente; RD, arts. 23 y 25.3) | Según la lectura de [R-6](ambiguedades.md#r-6): con AD-1, —; con AD-2, art. 77.1.b: la información y las circunstancias consideradas y los resultados. | `alerta_descartada` |
...
| Quién decide | RD, art. 25.2: el representante, o el órgano de control interno si el procedimiento lo prevé. | — (arts. 11.2 y 69.6: el responsable del cumplimiento normativo comunica y remite; ninguno dice quién decide: [R-4](ambiguedades.md#r-4)) | `decisor` |
...
| Intervención humana significativa | — | Art. 76.5.b, para las decisiones que enumera ([R-2](ambiguedades.md#r-2)). | `intervencion_humana` |
...
| Fecha de aplicación | La Ley y el RD están vigentes; ninguno de los dos textos consolidados menciona el AMLR ([R-3](ambiguedades.md#r-3)). | Art. 90: 10 de julio de 2027; 10 de julio de 2029 para las entidades del art. 3, punto 3, letras n) y o). | `sujeto.actividad` |

**Lo que resulta de la tabla.** El RD pide más datos formales del expediente (fechas, motivo, fases, fuentes, razones, quién decide, votos). El AMLR pide menos, pero pide dos cosas que el RD no tiene: las «circunstancias consideradas» y la copia de la comunicación. Y, con la lectura AD-2 de [R-6](ambiguedades.md#r-6), pide registrar alertas que en España no dejan registro. Además, solo el AMLR pone condiciones al uso de sistemas automatizados o de IA.
...
| V-6 | `fecha_apertura` ≤ `fecha_fin_analisis_tecnico` ≤ `decision_comunicacion.fecha` ≤ `comunicacion.fecha`, y `fecha_apertura` ≤ `fecha_cierre`. | No se puede decidir antes de analizar, ni comunicar antes de decidir. No se comprueba el orden entre `fecha_cierre` y la decisión o la comunicación ([R-9](ambiguedades.md#r-9)). |
...
| V-9 | No se comprueba que los votos den la mayoría que refleja `comunicar`. | Es un hecho que el cálculo debe señalar, y cómo cuentan las abstenciones no está resuelto ([R-8](ambiguedades.md#r-8)). |
...
| V-11 | No se comprueba el orden de las fechas de las participaciones y de las intervenciones humanas respecto de la apertura o de la decisión. | Una alerta es anterior a la apertura, y una intervención posterior a la decisión es precisamente lo que el cálculo tiene que poder señalar ([R-2](ambiguedades.md#r-2)). |
...
Los casos R-1 a R-11 están en [`ambiguedades.md`](ambiguedades.md), cada uno con qué dice la norma, por qué no determina un comportamiento único, qué hace la implementación, cómo se señala en la salida y a qué régimen afecta. Los enlaces «R-n» de este documento llevan a ese fichero.
